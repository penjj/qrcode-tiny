...
function toBytes(code: number, bits: number = 8) {
  if (bits === 0)
    return []
  return [...code.toString(2).padStart(bits, '0')].map(Number)
}

export function encodeText(text: string, ecc: Ecc) {
  if (!text)
    return []
...
export function createSegment(mode: Mode, numChars: number, bitData: readonly number[]) {
  return {
    mode,
    numChars,
    bitData,
...
export function getNumDataCodewords(ver: number, ecl: Ecc): number {
  return (
    floor(getNumRawDataModules(ver) / 8) - ecl.perBlocks[ver] * ecl.correctBlocks[ver]
  )
}

function getTotalBits(seg: Segment, version: number): number {
  const ccBits = version > 26 ? 16 : 8
...
export function encodeSegments(
  ...
  const bb: number[] = [
    ...toBytes(seg.mode.bits, 4),
    ...toBytes(seg.numChars, version > 26 ? 16 : 8),
    ...seg.bitData,
  ]
  ...
  bb.push(
    ...toBytes(0, min(4, dataCapacityBits - bb.length)),
  )
  bb.push(
    ...toBytes(0, (8 - (bb.length % 8)) % 8),
  )
  assert(bb.length % 8 === 0)

  // Pad with alternating bytes until data capacity is reached
  for (let padByte = 0xEC; bb.length < dataCapacityBits; padByte ^= 0xEC ^ 0x11)
    bb.push(...toBytes(padByte, 8))
  ...
  return createQrCode(version, ecl, dataCodewords, -1)
}
...
export function createQrCode(version: number, ecc: Ecc, dataCodewords: number[], msk: number) {
  ...
  function drawFinderPattern(x: number, y: number): void {
    for (let dy = -4; dy <= 4; dy++) {
      for (let dx = -4; dx <= 4; dx++) {
        const dist: number = max(abs(dx), abs(dy)) // Chebyshev/infinity norm
  ...
  function getAlignmentPatternPositions(): Array<number> {
    if (version === 1) { return [] }
    else {
      const numAlign: number = floor(version / 7) + 2
      const step: number
  ...
  function drawFunctionPatterns(): void {
    ...
    const alignPatPos: Array<number> = getAlignmentPatternPositions()
    ...
    function drawAlignmentPattern(x: number, y: number): void {
  ...
  function drawVersion(): void {
    ...
    let rem: number = version // version is uint6, in the range [7, 40]
    for (let i = 0; i < 12; i++) rem = (rem << 1) ^ ((rem >>> 11) * 0x1F25)
    const bits: number = (version << 12) | rem // uint18
    assert(bits >>> 18 === 0)

    // Draw two copies
    for (let i = 0; i < 18; i++) {
      const color: boolean = getBit(bits, i)
      const a: number = size - 11 + (i % 3)
      const b: number = floor(i / 3)
  ...
  function drawFormatBits(mask: number): void {
    // Calculate error correction code and pack bits
    const data: number = (ecc.formatBit << 3) | mask // errCorrLvl is uint2, mask is uint3
    let rem: number = data
  ...
  function addEccAndInterleave(data: Readonly<number[]>): number[] {
    const ver: number = version
    if (data.length !== getNumDataCodewords(ver, ecc))
      throw new _RangeError('Invalid argument')

    // Calculate parameter numbers
    const numBlocks: number = ecc.correctBlocks[ver]
    const blockEccLen: number = ecc.perBlocks[ver]
    const rawCodewords: number = floor(getNumRawDataModules(ver) / 8)
    const numShortBlocks: number = numBlocks - (rawCodewords % numBlocks)
    const shortBlockLen: number = floor(rawCodewords / numBlocks)
  ...
  function drawCodewords(data: Readonly<number[]>): void {
    ...
    for (let right = size - 1; right >= 1; right -= 2) {
      ...
      for (let vert = 0; vert < size; vert++) {
        ...
        for (let j = 0; j < 2; j++) {
          ...
          const y: number = upward ? size - 1 - vert : vert // Actual y coordinate
  ...
  function finderPenaltyCountPatterns(runHistory: Readonly<Array<number>>): number {
  ...
  function getPenaltyScore(): number {
    ...
    const total: number = size * size // Note that size is odd, so dark/total != 1/2
    // Compute the smallest integer k >= 0 such that (45-5k)% <= dark/total <= (55+5k)%
    const k: number = ceil(abs(dark * 20 - total * 10) / total) - 1
    assert(k >= 0 && k <= 9)
    result += k * PENALTY_N4
  ...
  const allCodewords: number[] = addEccAndInterleave(dataCodewords)
  ...
  if (msk === -1) {
    ...
    for (let i = 0; i < 8; i++) {
      applyMask(i)
      drawFormatBits(i)
      const penalty: number = getPenaltyScore()
...
function reedSolomonComputeDivisor(degree: number): Array<number> {
